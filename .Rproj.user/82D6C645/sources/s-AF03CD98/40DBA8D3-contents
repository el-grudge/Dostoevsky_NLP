...
d$id <- as.integer(rownames(d))
text_cleaning_tokens <- d %>% unnest_tokens(word, txt)
text_cleaning_tokens$word <- gsub('[[:digit:]]+', '', text_cleaning_tokens$word)
text_cleaning_tokens$word <- gsub('[[:punct:]]+', '', text_cleaning_tokens$word)
text_cleaning_tokens <- text_cleaning_tokens %>% filter(!(nchar(word) == 1))%>% anti_join(stop_words)
tokens <- text_cleaning_tokens %>% filter(!(word==""))
tokens <- tokens %>% mutate(ind = row_number())
tokens <- tokens %>% group_by(id) %>% mutate(ind = row_number()) %>% tidyr::spread(key = ind, value = word)
tokens [is.na(tokens)] <- ""
...
dtm <- CreateDtm(tokens$text, 
                 doc_names = tokens$id, 
                 ngram_window = c(1, 2))
#tm::DocumentTermMatrix(Corpus_crime_punishment_1)
...
final_summary_words <- final_summary_words %>% reshape::melt(id.vars = c("topic"))
...
# final_summary_words <- final_summary_words %>% group_by(topic,word) %>% arrange(desc(value))
...
for(i in 1:length(unique(final_summary_words$topic))){
  wordcloud(words = subset(final_summary_words ,topic == i)$word, 
            ...
            freq = as.integer(subset(final_summary_words ,topic == 1)$topic),
            min.freq = 1,
            max.words=200, 
            random.order=FALSE,
            rot.per=0.35,
