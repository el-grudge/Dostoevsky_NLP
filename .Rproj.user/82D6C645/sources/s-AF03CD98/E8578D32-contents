...
sentiments_afinn <- sentiments_afinn %>% 
  mutate(sentiment = ifelse(sentiments_afinn$value > 0, "positive", "negative"),
         lexicon = "AFINN") %>% 
  select(-value)

sentiments_bing <- get_sentiments("bing")
sentiments_bing <- sentiments_bing %>% mutate(lexicon = "Bing")

sentiments_nrc <- get_sentiments("nrc")
sentiments_nrc <- sentiments_nrc %>% mutate(lexicon = "NRC")

sentiments_M <- rbind(sentiments_afinn, sentiments_bing, sentiments_nrc)

new_sentiments <- sentiments_M %>% 
  group_by(lexicon) %>%
